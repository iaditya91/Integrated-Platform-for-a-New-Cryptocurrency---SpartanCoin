...
describe('Integration Test', () => {
    ...
    const createspartancoin = (name, host, port, peers, removeData = true) => {
    ...
    step('start server 1', () => {
        return createspartancoin(name1, 'localhost', 3001, [])
            .then((httpServer) => {
    ...
    step('mine an empty block', () => {
        return Promise.resolve()
            .then(() => {
                return supertest(context.httpServer1.app)
                    .post('/miner/mine/simplifiedpoet')
                    .send({ rewardAddress: context.address1 })
                    .expect(201);
            });
    });
    ...
    step('mine an empty block', () => {
        return Promise.resolve()
            .then(() => {
                return supertest(context.httpServer1.app)
                    .post('/miner/mine/enhancedsimplifiedpoet')
                    .send({ rewardAddress: context.address1 })
                    .expect(201);
            });
    });
    ...
    step('start server 2', () => {
        return createspartancoin(name2, 'localhost', 3002, [{ url: 'http://localhost:3001' }])
            .then((httpServer) => {
    ...
    describe('Complementary tests', () => {
        ...
        step('restart server 1', () => {
            return Promise.resolve()
                ...
                .then(() => {
                    return createspartancoin(name1, 'localhost', 3001, [], false)
                        .then((httpServer) => {
                            context.httpServer1 = httpServer;
                        });
                });
