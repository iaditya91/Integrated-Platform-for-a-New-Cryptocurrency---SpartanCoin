...
class PoWMiner {
    ...
    mine(rewardAddress, feeAddress) {
        let baseBlock = PoWMiner.generateNextBlock(rewardAddress, feeAddress, this.blockchain);
        ...
        const thread = spawn(function (input, done) {
            ...
            const {PoWMiner} = require(input.__dirname);
            /*eslint-enable */

            done(PoWMiner.proveWorkFor(Block.fromJson(input.jsonBlock), input.difficulty));
        });
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        const miningTime = Math.floor(Math.random() * (15-9))+9;
        ...
        // Add fee transaction (1 spartan per transaction)        
        if (transactions.length > 0) {
            let feeTransaction = Transaction.fromJson({
                ...
                data: {
                    ...
                    outputs: [
                        {
                            amount: Config.FEE_PER_TRANSACTION * transactions.length, // format
        ...
        if (rewardAddress != null) {
            let rewardTransaction = Transaction.fromJson({
                ...
                data: {
                    ...
                    outputs: [
                        {
                            amount: Config.MINING_REWARD, // format
...
class PoETMiner {
    ...
    mine(rewardAddress, feeAddress) {
        let baseBlock = PoETMiner.generateNextBlock(rewardAddress, feeAddress, this.blockchain);
        process.execArgv = R.reject((item) => item.includes('debug'), process.execArgv);
        ...
        const thread = spawn(function (input, done) {
            ...
            require(input.__dirname + '/../util/consoleWrapper.js')('mine-worker', input.logLevel);
            const Block = require(input.__dirname + '/../blockchain/block');
            const {PoETMiner} = require(input.__dirname);
            /*eslint-enable */

            done(PoETMiner.proveWorkFor(Block.fromJson(input.jsonBlock), input.difficulty));
        });
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        const index = previousBlock.index + 1;
        const previousHash = previousBlock.hash;
        const miningTime = Math.floor(Math.random() * (9-5))+5;
        const timestamp = new Date().getTime() / 1000;
        const blocks = blockchain.getAllBlocks();
        const candidateTransactions = blockchain.transactions;
        const transactionsInBlocks = R.flatten(R.map(R.prop('transactions'), blocks));
...
class EnhancedPoETMiner {
    ...
    mine(rewardAddress, feeAddress) {
        let baseBlock = EnhancedPoETMiner.generateNextBlock(rewardAddress, feeAddress, this.blockchain);
        process.execArgv = R.reject((item) => item.includes('debug'), process.execArgv);
        ...
        const thread = spawn(function (input, done) {
            ...
            require(input.__dirname + '/../util/consoleWrapper.js')('mine-worker', input.logLevel);
            const Block = require(input.__dirname + '/../blockchain/block');
            const {EnhancedPoETMiner} = require(input.__dirname);
            /*eslint-enable */

            done(EnhancedPoETMiner.proveWorkFor(Block.fromJson(input.jsonBlock), input.difficulty));
        });
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        const index = previousBlock.index + 1;
        const previousHash = previousBlock.hash;
        const miningTime = Math.floor(Math.random() * (7-3))+3;
        const timestamp = new Date().getTime() / 1000;
        const blocks = blockchain.getAllBlocks();
        const candidateTransactions = blockchain.transactions;
        const transactionsInBlocks = R.flatten(R.map(R.prop('transactions'), blocks));
        ...
        if (transactions.length > 0) {
            let feeTransaction = Transaction.fromJson({
                ...
                data: {
                    ...
                    outputs: [
                        {
                            amount: Config.FEE_PER_TRANSACTION * transactions.length, // spartans format
                            address: feeAddress, // INFO: Usually here is a locking script (to check who and when this transaction output can be used), in this case it's a simple destination address 
                        }
                    ]
                }
...
module.exports = {PoWMiner, PoETMiner, EnhancedPoETMiner};
