...
router.post("/signUp", async (req, res) => {
  try {
    ...
    if (req.body.username.length < 6)
      return res
        .status(400)
        .json({ message: "Your Username should be at least 6 characters." });
    if (req.body.password.length < 6)
      return res
        .status(400)
        .json({ message: "Your Password should be at least 6 characters." });
    ...
    User.find({ username: req.body.username }, (err, userData) => {
      ...
      if (err) {
        ...
        if (err) {
          return res
            .status(400)
            .json({ message: "Error decrypting Password." });
        }
    ...
    User.deleteOne({ _id: req.session.user_id })
      .then((_) => {
        req.session.destroy(() => {
          res
            .status(200)
            .json({ message: "User's account has been successfully deleted." });
        });
      })
      .catch((err) => {
